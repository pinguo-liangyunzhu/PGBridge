...
;(function(){
	...
	a.checkJsApi = function(data, callback){
		...
		if (data.jsApiList) {
			...
			if (data.jsApiList.length <= 0) {throw new Error('checkJsApi list is empty!')};
...
(function(){
	...
	function init(interfaces, messageHandler) {
		...
		if (WebViewJavascriptBridge._messageHandler) { throw new Error('PGBridge.init called twice'); };
